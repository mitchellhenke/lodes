...
aggregate_lodes <- function(year, state, geography, origin, save = T){
  ...
  # aggregate to each supplied geography and save, arranged by each supplied origin
  for(i in seq(geography)){
    ...
    fips.length <- case_when(
      geography[i] == "county" ~ 5,
      geography[i] == "tract" ~ 11,
      geography[i] == "block_group" ~ 12
    )
    
    print(paste("aggregating", state, year, geography[i]))
    ...
    if(save == T){
      for(j in seq(origin)){
        dir.create(path = paste0("intermediate/od_lodes/year=", year, "/geography=",
                                 geography[i], "/origin=", origin[j], "/state=", state),
                   showWarnings = F, recursive = T)
        
        print(paste("saving", state, year, geography[i], origin[j]))
        
        lodes.agg |>
          filter(str_sub(!!rlang::sym(origin[j]), 1, 2) == st.fips) |>
          arrange(across(!!rlang::sym(origin[j]))) |>
